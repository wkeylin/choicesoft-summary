...
* _.map(collection, [iteratee=_.identity])
    ...
    ```
## 疑惑  
* 用户做一些修改、删除、添加等操作后直接进行重新向后台请求列表，以达到列表为最新的数据的目的，但是这样做一是每一次操作都会增加一次请求服务器次数；并且新添加的数据无法直观的展现在列表中，经常添加的数据还要去分页查找。为什么不直接添加后在list数组中用unshift在头部添加到第一行，并且可以结合动画